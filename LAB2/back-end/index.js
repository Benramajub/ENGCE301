...
server.route({
  ...
  handler: async function (request, reply) {
    const delete_text = request.query.delete_text
    ...
    try {
      ...
      if (responsedata.error) {
        return reply.response({ message: responsedata.errMessage }).code(404)
      } else {
        return reply.response({ message: responsedata.message }).code(200)
      }
    } catch (err) {
      server.log(['error', 'home'], err)
