...
async function deleteMovie(delete_text) {
    ...
    return new Promise((resolve, reject) => {
      Query = 'DELETE FROM movies WHERE title LIKE ?'
  
  
      pool.query(Query, [`%${delete_text}%`], function (error, results, fields) {
        if (error) {
          pool.end()
          return reject({ error: true, errMessage: 'Database query error' })
        }
